...
## plot and store the four graphics into one screen
par(mfrow=c(2,2),mar=c(4,4,2,1))
with(GAP.data,{
        ## graph upper right hand (URH)
        plot(GAP.data$Global_active_power~GAP.data$DT, type="l",
             ylab="Global Active Power (kilowatts)", xlab="")
        ## graph upper left hand (ULH)
        plot(GAP.data$Voltage~GAP.data$DT, type="l",
             ylab="Voltage", xlab="datetime")
        ##graph lower left hand (LLH)
        ...
        ## graph lower right hand (LRH)
        plot(GAP.data$Global_reactive_power~GAP.data$DT, type="l",
            xlab="datetime",ylab="Global_reactive_power")
})

## store the graphic to file
